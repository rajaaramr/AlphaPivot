...
BASE_INTERVAL = os.getenv("BASE_INTERVAL", "15m")  # preferred base granularity to read from DB
...
def load_vpbb_cfg(ini_path: str = DEFAULT_INI) -> VPBBConfig:
    # defaults tuned for the new 15/30/60/120/240 flow
    dflt = {
        "lookback_days": 7,
...
def _secs(rule: str | None) -> int | None:
    ...
    off = to_offset(rule)
    return int(off.delta.total_seconds())
...
# ---------------------------------------------------------------------
# IO: load intraday candles with fallback (BASE_INTERVAL → 5m)
# ---------------------------------------------------------------------
def load_intra(symbol: str, kind: str, *, lookback_days: int) -> pd.DataFrame:
    ...
    tried = set()
    # Try BASE_INTERVAL first, then 5m, then a couple common ones
    order = [BASE_INTERVAL, "5m", "15m", "30m", "60m"]
    order = [iv for iv in order if iv not in tried and iv in TF_TO_OFFSET]  # de-dup & valid only
...
def _dynamic_volpct_threshold(dftf: pd.DataFrame, base_low: int, base_high: int) -> int:
    ...
    w = 1.0 / (1.0 + np.exp(-z))  # higher in high-vol regimes
...
def _auto_thresholds(dftf: pd.DataFrame, base: dict, *, cfg: VPBBConfig) -> dict:
    ...
    # approximate "days*bars" mapping for 5m base; clamped robustly
    win_long = min(len(vol), cfg.std_lookback_days * max(1, int(24*60/5)))
    obv = _obv_series(cl, vol)
    obv_delta = obv.diff().fillna(0.0)

    _ = _rolling_std(vol, win=win_long)   # kept for parity / potential future use
...
# ---------------------------------------------------------------------
# zone_levels upsert (RSI/MFI snapshot aware of market kind)
# ---------------------------------------------------------------------
def _get_rsi_mfi_at(cur, symbol: str, kind: str, tf: str, ts: datetime) -> tuple[Optional[float], Optional[float]]:
    cur.execute("""
        SELECT metric, val
          FROM indicators.values
         WHERE symbol=%s AND market_type=%s AND interval=%s AND ts=%s
...
def _upsert_zone_levels_row(
    ...
    """
    with get_db_connection() as conn, conn.cursor() as cur:
        rsi, mfi = _get_rsi_mfi_at(cur, symbol, kind, tf, ts)
        cur.execute(sql, (
            symbol, tf, ts,
...
def process_symbol(symbol: str, *, cfg: Optional[VPBBConfig] = None) -> int:
    ...
    df = load_intra(symbol, cfg.market_kind, lookback_days=cfg.lookback_days)
    if df.empty:
        print(f"⚠️ {cfg.market_kind}:{symbol} no intraday data (tried {BASE_INTERVAL} → 5m) in last {cfg.lookback_days}d")
        return 0
    ...
    for tf in cfg.tf_list:
        ...
        try:
            _upsert_zone_levels_row(symbol, cfg.market_kind, tf, ts_last, vp, bbres)
